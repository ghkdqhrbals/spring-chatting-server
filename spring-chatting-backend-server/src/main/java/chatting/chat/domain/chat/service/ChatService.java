...
public class ChatService {
    ...
    public Chatting findById(Long id) {
        return chatRepository.findById(id)
            .orElseThrow(() -> new CustomException(CANNOT_FIND_CHATTING));
    }
    ...
    public ChatRecordDTO save(Chatting chatting) throws CustomException {
        Participant findParticipant = participantRepository.findByRoomIdAndUserId(
            chatting.getRoom().getRoomId(), chatting.getSendUser().getUserId());
        // 채팅방 참여인원인지 확인
